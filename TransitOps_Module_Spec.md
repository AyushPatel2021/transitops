...
| **Phase 3 — Trip Engine** | 2:00–3:10 (70m) | Trip create/dispatch/complete/cancel, all validation rules (§3.4), domain filters on dropdowns | ✅ Done — Trip creation auto-sets `created_by` for authenticated creates, validates cargo capacity on create/update, filters vehicle/driver dropdowns for dispatch eligibility, blocks direct status edits, dispatches available assets, prevents double-booking at dispatch, completes trips with final readings and automatic fuel log creation, restores assets on cancel, blocks terminal transitions, and includes Phase 3 lifecycle tests |
| **Phase 4 — Maintenance + Finance** | 3:10–4:00 (50m) | Maintenance log + auto in_shop/available toggling, Fuel logs, Expenses, cost roll-up computation | ✅ Done — Maintenance creation auto-sets `created_by`, starts available vehicles into `in_shop`, blocks multiple active logs, closes with required `end_date`, restores non-retired vehicles to available, fuel and expense values are validated, vehicle operational cost rolls up fuel plus maintenance costs, and Phase 4 tests cover the workflow |
